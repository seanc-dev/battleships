    ...
    while True:
        try:
            ...
            print("Make sure you enter a number between {0} and {1}.".format(
                str(board_dimensions["board_min"]), str(board_dimensions["board_max"])))
            continue
        else:
            return guess


def shot_outcomes(shot, game_variables):
    outcomes = {'all_ships_sunk': False, 'all_guesses_used': False, 'hit': False, 'ship': {}, 'section': {}}
    ships = game_variables["ships"]
    for ship in ships:
        for ship_section in range(ships[ship]["length"]):
            if validate.row_and_col_match(shot, ships[ship][ship_section]):
                ...
                if ships[ship]["SectionsRemaining"] - 1 == 0:
                    ...
                    if game_variables["ships_remaining"] - 1 == 0:
                        outcomes["all_ships_sunk"] = True
                return outcomes
    if game_variables["misses_remaining"] - 1 == 0:
...
def shot_hit_flow(outcomes, ships, game_variables):
    ...
    game_variables["all_ships_sunk"] = outcomes["all_ships_sunk"]
